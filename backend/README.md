...
| Méthode | Route | Rôle |
|---|---|---|
| POST | `/api/v1/devices` | Enregistrer l'appareil push (idempotent, ré-attribution) |
| DELETE | `/api/v1/devices/{token}` | Désenregistrer à la déconnexion (204 / 404) |
| GET | `/api/v1/notifications` | Cloche : 50 dernières + `unread_count` |
| GET | `/api/v1/notifications/count` | Badge léger (polling) |
| POST | `/api/v1/notifications/{id}/read` | Marquer lu (propriétaire only) |
| POST | `/api/v1/notifications/read-all` | Tout marquer lu |
| GET/PUT | `/api/v1/notifications/prefs` | Préférences (`matches`, `messages`) |

Configuration : `PUSH_PROVIDER=log|fcm` (+ `FCM_SERVER_KEY` en prod — jamais côté client).
Événements notifiés : match créé (les deux membres) et message reçu (anti-spam par conversation).


### Sauvegardes & sécurité (Phase 9)
```bash
# Sauvegarde (gzip + sha256 + rotation) — backups/ est git-ignoré
./.venv/bin/python tools_backup_db.py --keep 14
# Restauration DÉFINITIVE (checksum vérifié, --force requis)
./.venv/bin/python tools_restore_db.py backups/fasolove-AAAAMMJJ-HHMMSS.sql.gz --force
# Certification E2E de la chaîne (sandbox uniquement)
./.venv/bin/python tools_e2e_backup.py
# Audit dépendances (déployé) : 0 vulnérabilité connue au 25/09/2026
./.venv/bin/pip-audit
```
Durcissement réseau en production (`ENV=prod`) : `/docs`+`/openapi.json`
désactivés, HSTS activé, `Permissions-Policy`, `Cache-Control: no-store` sur
auth/comptes, garde-fou de démarrage (variables restées DEV journalisées).
Voir `../docs/SECURITY_AUDIT.md` et `../docs/RUNBOOK_OPERATIONS.md`.
