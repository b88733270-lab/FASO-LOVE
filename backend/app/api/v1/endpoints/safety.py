...
async def delete_my_account(
    ...
    # Monétisation : abonnements (FK vers transactions) puis transactions.
    await db.execute(delete(Subscription).where(Subscription.user_id == me.id))
    await db.execute(delete(PaymentTransaction).where(PaymentTransaction.user_id == me.id))
    # Notifications (Phase 8) : cloche + appareils push.
    await db.execute(delete(Notification).where(Notification.user_id == me.id))
    await db.execute(delete(PushDevice).where(PushDevice.user_id == me.id))
...
async def export_my_data(
    ...
    transactions = (
        (
            await db.execute(
                select(PaymentTransaction).where(PaymentTransaction.user_id == me.id)
            )
        )
        .scalars()
        .all()
        ...
        "abonnements": [
            {
                "offre": s.plan_code,
                "statut": s.status,
                "debut": s.starts_at.isoformat(),
                "fin": s.ends_at.isoformat(),
            }
