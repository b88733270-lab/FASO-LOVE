...
class Settings(BaseSettings):
    ...
    # Quotas FREEMIUM quotidiens (Premium = illimité).
    free_likes_per_day: int = 30
    free_super_likes_per_day: int = 1
    # Prestataire actif : "mock" (démo/certification sandbox, aucun vrai
    # prélèvement) ou "paydunya" (agrégateur Orange Money + Moov, BF).
    payments_provider: str = "mock"
    # Bouton « j'ai payé » de démo — JAMAIS actif en production.
    payments_simulation_enabled: bool = True
    # Agrégateur PayDunya (https://paydunya.com — couvre Orange Money et
    # Moov Money Burkina Faso) : clés à provisionner dans le vault prod.
    paydunya_master_key: str = ""
    paydunya_private_key: str = ""
    paydunya_public_key: str = ""

    # --- Notifications (Phase 8) ---
    # Prestataire push : "log" (démo/certification, aucun envoi réel) ou
    # "fcm" (Firebase Cloud Messaging, nécessite la clé serveur dans le
    # vault — le jeton vaudou n'existe plus ; le client app enregistre
    # toujours ses appareils via /devices quel que soit le mode).
    push_provider: str = "log"
    fcm_server_key: str = ""

    # --- Observabilité (Phase 10) ---
    # Sentry (traçage des exceptions serveur) : importé UNIQUEMENT si le DSN
    # est posé ; le SDK est facultatif à l'installation (requirements-dev/
    # prod géré par l'ops) — l'app tourne sans.
    sentry_dsn: str = ""
    sentry_environment: str = ""  # défaut = env
    paydunya_token: str = ""
    paydunya_sandbox: bool = True
    # Stripe (diaspora, carte bancaire) — Phase 7b.
