...
async def main() -> None:
    async with AsyncClient(base_url=BASE_URL, timeout=30) as c:
        ...
        print("— Notifications (cloche in-app + appareils push)")
        # Enregistrement d'un appareil FCM (sandbox : push journalisé côté serveur).
        r = await c.post(f"{API}/devices", headers=h_a,
                         json={"platform": "android", "token": "fcm-e2e-android-00000000001"})
        check(r.status_code == 201, "appareil push enregistré")
        device_id = r.json()["id"]
        r = await c.post(f"{API}/devices", headers=h_a,
                         json={"platform": "android", "token": "fcm-e2e-android-00000000001"})
        check(r.json()["id"] == device_id,
              "appareil réenregistré idempotent (même appareil)")
        # Le match du scénario + les messages ont rempli la cloche des deux côtés.
        data = (await c.get(f"{API}/notifications", headers=h_a)).json()
        kinds = [n["kind"] for n in data["items"]]
        check("match" in kinds and "message" in kinds,
              "cloche remplie par le match ET le message 💬")
        check(data["unread_count"] >= 2, "badge non-lues cohérent")
        badge = (await c.get(f"{API}/notifications/count", headers=h_a)).json()
        check(badge["unread_count"] == data["unread_count"], "compteur léger = badge")
        # Préférences : B coupe les messages → le prochain message ne badge plus.
        await c.put(f"{API}/notifications/prefs", headers=h_b,
                    json={"matches": True, "messages": False})
        before = (await c.get(f"{API}/notifications/count", headers=h_b)).json()["unread_count"]
        await c.post(f"{API}/matches/{match_id}/messages", headers=h_a,
                     json={"content": "Message sous préférence coupée"})
        after = (await c.get(f"{API}/notifications/count", headers=h_b)).json()["unread_count"]
        check(after == before, "préférence « messages » respectée (aucun badge)")
        await c.put(f"{API}/notifications/prefs", headers=h_b,
                    json={"matches": True, "messages": True})
        # Lecture globale : le badge de A retombe à zéro.
        r = await c.post(f"{API}/notifications/read-all", headers=h_a)
        check(r.json()["updated"] >= 1, "tout-marquer-lu appique")
        check((await c.get(f"{API}/notifications/count", headers=h_a)).json()["unread_count"] == 0,
              "badge à zéro après lecture globale")
