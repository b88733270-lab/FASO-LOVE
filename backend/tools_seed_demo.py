...
from app.models.payments import PaymentTransaction, Subscription  # noqa: E402
...
async def seed() -> int:
    ...
    async with AsyncSessionLocal() as db:
        ...
        # Démonstration Premium : Idrissa a un abonnement actif payé (mock).
        if idrissa:
            from sqlalchemy import select as _select

            has_sub = await db.scalar(
                _select(Subscription).where(Subscription.user_id == idrissa)
            )
            if has_sub is None:
                now = utcnow()
                txn = PaymentTransaction(
                    ...
                    amount_fcfa=3500,
                    phone_e164="+22670000005",
                    status="succeeded",
                    provider_ref="mock-seed-idrissa",
                    provider_event_id="seed-1",
                    succeeded_at=now,
                )
                db.add(txn)
                await db.flush()
