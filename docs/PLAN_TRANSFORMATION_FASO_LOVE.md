...
| **5 — Signalement, blocage, admin v1** ✅ **TERMINÉE côté API (24 sept. 2025)** | Reports (motifs FR dont arnaque/mineur) + blocks symétriques ; API `/admin` (stats, file signalements, gestion utilisateurs, modération photos) ; suppression/export compte — *Report : écran admin Flutter Web (phase 5 restante), journaux d'audit persistants* | Outils de modération (exigence stores) |
...
| **7 — Monétisation** (≈ 2 sem.) | Quotas freemium ; Premium ; intégration Mobile Money (PayDunya/LigdiCash/CinetPay) + Stripe diaspora ; webhooks | Revenus activables |
