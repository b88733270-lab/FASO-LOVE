...
**Engagement**
16. Notifications : ✅ cloche in-app persistante + push FCM-ready (match/message, préférences serveur, anti-spam, parc d'appareils) — **Phase 8 livrée** ; reste : projet Firebase réel + SMS de re-engagement (budget maîtrisé)
...
| **10 — Livraison finale** ✅ **TERMINÉE (25 sept. 2026)** | Docker prod non-root + pile compose (api/pg/Caddy TLS auto), CI GitHub Actions 4 étages bloquants (pytest, pip-audit 0-CVE, Dart, E2E fumée), Sentry branchable par DSN, probes /health/live/ready, `docs/LIVRAISON_FINALE.md` (stores, fiches incident), versions 1.0.0 verrouillées — **Produit fini ; restent les étapes humaines** 🔧 (RCCM, comptes prestataires, VM+DNS, listing stores) | **Production v1.0** |
