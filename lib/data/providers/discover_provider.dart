...
class DiscoverProvider with ChangeNotifier {
  ...
  /// Rappel déclenché quand le serveur bloque sur le quota freemium
  /// (403 « Quota quotidien atteint… Premium »).
  void Function()? onQuotaAtteint;
    ...
    if (!horsLigne) {
      try {
        ...
        if (e.statusCode == 403 && e.message.contains('Premium')) {
          onQuotaAtteint?.call();
          return; // la carte n'est pas consommée : retry possible après upgrade
        }
        erreur = e.message;
      }
    }
